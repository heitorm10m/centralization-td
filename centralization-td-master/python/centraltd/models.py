...
FRAME_METHOD_DAO_EQ11 = "Dao et al. Eq. 11"
...
def _canonical_frame_method(value: str | None) -> str:
    if value is None:
        return FRAME_METHOD_DAO_EQ11
    normalized = value.strip()
    if normalized in {"parallel-transport", "dao-et-al-eq-11", FRAME_METHOD_DAO_EQ11}:
        return FRAME_METHOD_DAO_EQ11
    return normalized
...
class Nodal6DOFOutputModel:
    ...
    @classmethod
    def from_sequence(cls, values: Sequence[float], *, context: str) -> "Nodal6DOFOutputModel":
        if len(values) != 6:
            raise ValueError(f"{context} must contain exactly 6 DOFs: [u, v, w, theta_x, theta_y, theta_z].")
        return cls(
            u_m=float(values[0]),
            v_m=float(values[1]),
            w_m=float(values[2]),
            theta_x_rad=float(values[3]),
            theta_y_rad=float(values[4]),
            theta_z_rad=float(values[5]),
        )


def require_nodal_output_6dof(segment: Any, *, context: str) -> Nodal6DOFOutputModel:
    value = getattr(segment, "solved_nodal_output_6dof", None)
    if value is None:
        raise ValueError(
            f"{context} requires segment.solved_nodal_output_6dof with 6 DOFs [u, v, w, theta_x, theta_y, theta_z]."
        )
    if not isinstance(value, Nodal6DOFOutputModel):
        raise ValueError(
            f"{context} expected segment.solved_nodal_output_6dof to be a Nodal6DOFOutputModel, got {type(value).__name__}."
        )
    return value
...
class CaseDefinition:
    ...
    frame_method: str = FRAME_METHOD_DAO_EQ11
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "CaseDefinition":
        ...
        return cls(
            ...
            frame_method=_canonical_frame_method(_optional_text(raw, "frame_method", FRAME_METHOD_DAO_EQ11)),
        )
    ...
    def validate(self) -> None:
        ...
        if self.frame_method != FRAME_METHOD_DAO_EQ11:
            raise ConfigError(
                "case.frame_method must currently resolve to 'Dao et al. Eq. 11' "
                "(legacy alias 'parallel-transport' is still accepted in YAML)."
            )
