...
from .models import require_nodal_output_6dof
...
def evaluate_body_torque_drag_contribution(
    ...
    *,
    ...
    _nodal_output_6dof = require_nodal_output_6dof(
        segment,
        context="evaluate_body_torque_drag_contribution",
    )
    # The reduced body torque/drag closure currently uses resultant contact
    # forces and an effective contact radius. Rotational DOFs are validated
    # here so they are not silently discarded by interface changes, but this
    # consumer does not yet apply theta_x/theta_y/theta_z directly.
