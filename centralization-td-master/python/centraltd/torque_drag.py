...
from .models import LoadedCase, StringSectionModel, require_nodal_output_6dof
...
from .torque_drag_centralizer import (
    ...
    require_centralizer_segment_6dof,
)
...
def run_torque_drag_baseline(
    ...
    *,
    ...
    for segment_index in range(len(mechanical_profile) - 1, -1, -1):
        ...
        _nodal_output_6dof = require_nodal_output_6dof(
            segment,
            context="run_torque_drag_baseline",
        )
        require_centralizer_segment_6dof(segment)
        # This reduced torque/drag propagation still uses resultant contact
        # forces plus effective radii. The 6-DOF nodal state is validated here
        # so the runtime never drops [u, v, w, theta_x, theta_y, theta_z]
        # silently while the legacy closures are still being transitioned.
